...
void write_data(const std::string file_name, const std::map< std::string, Student* >& data) {
    ...
    for ( auto it = data.begin(); it != data.end(); ++it ) {
        file_object << it->second->student_number << ";"
                    << it->first << ";"
                    << it->second->name << ";"
                    << it->second->phone_number << ";"
                    << it->second->email << ";"
                    << it->second->skype << std::endl;
    }
}
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                if ( !is_valid_phone_number(new_phone_number) ) {
                    ...
                    // change the number
                    student_numbers.at(parts[1])->phone_number = new_phone_number;
                    for ( auto student : user_ids ) {
                        if ( student.second->student_number == parts[1] ) {
                            student.second->phone_number = new_phone_number;
                        }
                    }
